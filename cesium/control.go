...
func (db *DB) ConfigureControlUpdateChannel(ctx context.Context, key ChannelKey) error {
	...
	// It's important to set the controlDigests key AFTER we create the writer, so that we
	// don't send the control transfer generated by opening this specific writer to
	// itself before it is ready to process it.
	db.mu.controlDigests.key = key
	if err != nil {
		return err
	}
	db.mu.controlDigests.inlet, db.mu.controlDigests.outlet = confluence.Attach[WriterRequest, WriterResponse](w, 100)
...
func (db *DB) updateControlDigests(
	...
	return signal.SendUnderContext(
		ctx,
		db.mu.controlDigests.inlet.Inlet(),
		WriterRequest{Command: WriterWrite, Frame: db.ControlUpdateToFrame(ctx, u)},
	)
}

func (db *DB) closeControlDigests() {
	// We need to do a careful mutex lock covers both the controlDigests key check and
	...
	db.mu.Lock()
	if db.mu.controlDigests.key == 0 {
		db.mu.Unlock()
		return
	}
	db.mu.controlDigests.key = 0
	db.mu.Unlock()
	db.mu.controlDigests.inlet.Close()
	confluence.Drain(db.mu.controlDigests.outlet)
	return
}

func (db *DB) digestsConfigured() bool { return db.mu.controlDigests.key != 0 }
...
func (db *DB) ControlUpdateToFrame(ctx context.Context, u ControlUpdate) Frame {
	...
	return Frame{
		Keys:   []ChannelKey{db.mu.controlDigests.key},
